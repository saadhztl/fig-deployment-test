...
export function isLanguageSupported(language: string): boolean {
  return languageService.isLanguageSupported(language);
}
...
export async function getPage<T>(url: string, pageType: string) {
  const query = stack
    ...
    .locale(languageService.getLanguage()) // Add locale specification
  ...
  if (result.entries) {
    ...
    addEditableTagsIfPreview(entry, 'page'); // Adding editable tags for live preview if enabled
...
export const getEntries = async <T>({
  contentTypeUid,
  referencesToInclude = '',
}: Pick<GetEntries, 'contentTypeUid' | 'referencesToInclude'>) => {
...
export const getEntryByUid = async ({
  ...
  try {
    ...
    const entry = await entryQuery.locale(languageService.getLanguage()).fetch();
...
export const getEntriesByUids = async <T>({
  ...
  try {
    ...
    let entryQuery = entry.locale(languageService.getLanguage()).query();
