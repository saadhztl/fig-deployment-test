...
import { IEventsLandingPage } from '@/lib/generated';
...
export default async function SlugPage(props: EventsPageProps) {
  const { params } = props;
  ...
  const urlPath = '/events'; // Root path for locale page

  return <SharedPageLayout urlPath={urlPath} pageContentTypeUID="events_landing_page" />;
}
...
export async function generateMetadata(props: EventsPageProps): Promise<Metadata> {
  ...
  try {
    const page = await getPage<IEventsLandingPage>(urlPath, 'events_landing_page');
