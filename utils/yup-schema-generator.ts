...
import { IFields } from '@/lib/generated';
import { VALIDATION_MESSAGES, VALIDATION_PATTERNS } from '@/constants/form-constants';
...
export class YupSchemaGenerator {
  ...
  generateSchema(fields: IFields[]): yup.ObjectSchema<any> {
    const shape = fields.reduce((acc, field) => {
      ...
      // Handle locations field (custom field type)
      if (field.locations) {
        acc[field.locations.name] = this.createLocationsValidation(field.locations);
      }
  ...
  private createInputValidation(input: IFields['input']): yup.StringSchema {
    ...
    if (input.input_type === 'tel') {
      ...
      schema = schema.matches(VALIDATION_PATTERNS.PHONE, message);
    }
  ...
  private createDropdownValidation(dropdown: IFields['dropdown']): yup.StringSchema {
    ...
    if (dropdown.options_group && dropdown.options_group.length > 0) {
  ...
  private createRadioCheckboxValidation(radioCheckbox: IFields['radio_checkbox']): yup.Schema {
    if (radioCheckbox.type === 'Checkbox') {
      // For checkboxes, we typically expect an array
  ...
  generateFieldSchema(field: IFields): yup.Schema | null {
    ...
    if (field.locations) {
      return this.createLocationsValidation(field.locations);
    }
