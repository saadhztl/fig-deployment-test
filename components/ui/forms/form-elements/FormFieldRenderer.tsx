...
import { CSLPFieldMapping, IFields } from '@/lib/generated';
...
type FieldClassNames = {
  input?: { wrapper?: string; element: string };
  locations?: { wrapper?: string; element: string };
      ...
      {fields?.map((field, index) => {
        ...
        if (field.locations) {
          return (
            <LocationsDropdown
              key={index}
              location_name_type={field.locations.location_name_type}
              error={errors[field.locations.name]?.message as string}
              className={cn(
                defaultFieldClasses.locations?.element,
                fieldClassNames.locations?.element
              )}
              id={field.locations.name}
              wrapperClassName={fieldClassNames.locations?.wrapper}
              {...register(field.locations.name)}
              {...field.locations}
            />
          );
        }
        ...
        if (field.submit) {
          return (
            ...
            >
              ...
              <button
                type="button"
                className={cn(defaultFieldClasses.submit?.element, fieldClassNames.submit?.element)}
